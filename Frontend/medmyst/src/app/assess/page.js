...
export default function AssessPage() {
  ...
  return (
    <section className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded-2xl shadow">
      ...
      {step === 1 && <DemographicsForm onNext={next} />}
      {step === 2 && <SymptomsForm onNext={next} onBack={back} />}
      {step === 3 && <MedsAllergiesForm onNext={next} onBack={back} />}
      {step === 4 && <ConsentReview onBack={back} />}
    </section>
  );
}
